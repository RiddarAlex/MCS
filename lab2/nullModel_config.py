...
realNetwork_deg_seq = [d for n, d in realNetwork.degree()]
...
for iteration in range(iterations):
    ...
    G = nx.configuration_model(realNetwork_deg_seq,
                               create_using=type(realNetwork))
                               #change from multiGrapg default
...
print(f"=> real network is {(realGCC-avg_randomGCC)/standard_deviation} standard deviations above the config mean.")

boxes = [GCC_list, realGCC]

plt.boxplot(boxes, labels=["Configuration","Airlines"])

plt.title("Box plots of GGC for graph configuration and real network")
plt.ylabel("GCC")
plt.show()
