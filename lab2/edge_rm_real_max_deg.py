...
for alpha_index, alpha in enumerate(alpha_list):
    ...
    for j in range(nRuns):
        ...
        for i in range(int(ne*alpha)):
            ...
            max_vertex = max(degree_dict, key = lambda x: degree_dict[x])

            edges = list(G.edges(max_vertex))

            index = random.randrange(len(edges)-1)
    ...
    print(f" avg Global Clustering Coefficient: {gcc/nRuns} alpha = {alpha}")
...
plt.title('Average GCC as afunction of alpha with random edge removal')
plt.ylabel('Avg GCC')
plt.xlabel('alpha')
