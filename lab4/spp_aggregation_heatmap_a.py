...
#returns relative average position of neighbors in attraction zone
def get_avgPostition(particles, r1, r2, x0, y0):
...
if __name__ == '__main__':
    ...
    number_of_ac = 10
    attraction_coefficient_list = [0.1*i for i in range(number_of_ac)]


    attraction_coefficient = []

    aggregation_arr = []
    attraction_coefficient_arr = []
    for ac_index, attraction_coefficient in enumerate(attraction_coefficient_list):
        print(f"attraction_coefficient: {attraction_coefficient}")
        ...
        for iteration in range(iterations):
            ...
            while t < T:
                ...
                for i, (x, y) in enumerate(particles):
                    ...
                    avgPosition = get_avgPostition(particles, r, r_attraction, x, y)

                    attraction_angle = attraction_coefficient * vector_2_angle(avgPosition)

                    # get average theta angle
                    avg = get_average(thetas, neighbors)

                    # angle = (attraction_angle + avg) / 2
                    angle = (attraction_angle) * attraction_angle + (1 - attraction_angle) * avg
                    ...
                    if particles[i, 0] > domain_sideLength:
                        particles[i, 0] = particles[i, 0] - domain_sideLength

                    if particles[i, 1] < 0:
                        particles[i, 1] = 1 + particles[i, 1]

                    if particles[i, 1] > domain_sideLength:
                        particles[i, 1] = particles[i, 1] - domain_sideLength
            ...
            for i, (x, y) in enumerate(particles):
                neighbors = get_neighbors(particles, r, x, y)
                No_neigbors.append(len(neighbors))

            aggregation_score = mean(No_neigbors)

            aggregation_arr.append(aggregation_score)
            attraction_coefficient_arr.append(attraction_coefficient)

            print(f"Aggregation score: {aggregation_score}")

    
    fig_polarization = plt.subplots(figsize =(10, 7))
    plt.hist2d(attraction_coefficient_arr, aggregation_arr, cmap="hot")
    plt.title(f"Heatmap of aggregation for iterations = {iterations} and t = {T}")
    plt.ylabel('Aggregation score')
    plt.xlabel('Attraction coefficient')
    plt.colorbar()
