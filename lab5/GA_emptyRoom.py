...
def main():
    ...
    for generation in range(NoGenerations):
        ...
        top40indices, = np.where(fitness_arr > np.sort(fitness_arr)[9])
        top10indices, = np.where(fitness_arr > np.sort(fitness_arr)[39])
        
        k = 0
        for i in top40indices:
            j = random.choice(top40indices)
